...
require_once __DIR__ . '/billing_system.php';
...
// Initialiser le logging pour l'interface COURSIER
$interface_start_time = initLogging('COURSIER');

// Log du démarrage de l'interface coursier
logInfo("Démarrage interface coursier", [
    'user_agent' => $_SERVER['HTTP_USER_AGENT'] ?? 'unknown',
    'session_id' => session_id(),
    'user_authenticated' => isset($_SESSION['user_id']) ? 'yes' : 'no'
], 'COURSIER');

// Configuration base de données UNIFIÉE

// Configuration Google Maps API
define('GOOGLE_MAPS_API_KEY', 'AIzaSyBjUgj9KM0SNj847a_bIsf6chWp9L8Hr1A'); // API Complete Address
define('GOOGLE_PLACES_API_KEY', 'AIzaSyAGKC21fGmY-k6i0dcY8MpBExa5IqqBXbE'); // API Places

// ============================================================================
// MOTS DE PASSE UNIFIÉS - UTILISATION DE LA FONCTION GENERATEPASSWORD
// ============================================================================
// La fonction generatePassword() est désormais définie dans config.php et disponible globalement
// FONCTION DUPLIQUÉE SUPPRIMÉE

/**
 * Fonction de génération de mot de passe LOCAL pour coursier.php
 * Redirige vers la fonction unifiée ou utilise la même logique
 */

// Initialiser le système de facturation et CINETPAY
$billing = new SuzoskyBilling();
$cinetpay = new SuzoskyCinetPayIntegration();

// Log de l'initialisation des systèmes
logInfo("Systèmes initialisés", [
    'billing_system' => 'SuzoskyBilling',
    'payment_system' => 'SuzoskyCinetPayIntegration'
], 'COURSIER');

// Initialiser les tables de paiement (avec gestion d'erreur)
try {
    createPaymentTables();
} catch (Exception $e) {
    error_log("Erreur lors de l'initialisation des tables de paiement: " . $e->getMessage());
}

// Fonction d'optimisation de trajet
function optimizeDeliveryRoute($start_lat, $start_lng, $deliveries) {
    $api_key = GOOGLE_MAPS_API_KEY;
    
    if (empty($deliveries) || !$api_key) {
        return ['route' => [], 'total_distance' => 0, 'estimated_time' => 0];
    }
    
    // Construire les waypoints pour l'API Google Directions
    $waypoints = [];
    foreach ($deliveries as $delivery) {
        if ($delivery['latitude_pickup'] && $delivery['longitude_pickup']) {
            $waypoints[] = $delivery['latitude_pickup'] . ',' . $delivery['longitude_pickup'];
        }
        if ($delivery['latitude_delivery'] && $delivery['longitude_delivery']) {
    ...
    if ($data['status'] === 'OK' && !empty($data['routes'])) {
        ...
        // Réorganiser les livraisons selon l'ordre optimisé
        ...
        return [
            ...
            'total_distance' => $route['legs'][0]['distance']['value'] ?? 0,
            'estimated_time' => $route['legs'][0]['duration']['value'] ?? 0,
            'polyline' => $route['overview_polyline']['points'] ?? '',
            'optimized_order' => $optimized_order
        ];
    }
    
    return ['route' => $deliveries, 'total_distance' => 0, 'estimated_time' => 0, 'error' => 'Optimisation impossible'];
}

// Fonction pour assigner automatiquement les coursiers basée sur la géolocalisation
function assignNearestCourier($pickup_lat, $pickup_lng, $business_id = null) {
    try {
        $pdo = getDBConnection();
        
        // Rechercher les coursiers actifs avec leur position récente (moins de 1h)
        $stmt = $pdo->prepare("
        ...
        $coursiers_proches = $stmt->fetchAll();
        
        if (!empty($coursiers_proches)) {
            // Retourner le coursier le plus proche
            return $coursiers_proches[0];
        }
        
        return null;
        
    } catch (Exception $e) {
        error_log("Erreur assignation coursier: " . $e->getMessage());
        return null;
    }
}
...
function geocodeAddress($address) {
    $api_key = GOOGLE_MAPS_API_KEY;
    if (!$api_key) return ['lat' => null, 'lng' => null];
    ...
    $response = @file_get_contents($url);
    if ($response === false) return ['lat' => null, 'lng' => null];
...
function authenticateUser($identifier, $password, $pdo) {
    try {
        ...
        error_log("AUTH_DEBUG: Identifier='$identifier', Password length=" . strlen($password));
        ...
        if ($user) {
            error_log("AUTH_DEBUG: Agent trouvé - Matricule=" . $user['matricule']);
        ...
        if ($user && !empty($user['password'])) {
            ...
            if (password_verify($password, $user['password'])) {
                ...
                // Pas de fallback vers password_plain car cette colonne n'existe pas
            }
        } else {
            error_log("AUTH_DEBUG: Password vide en base");
        }
...
if ($_POST['action'] ?? '' === 'login') {
    ...
    error_log("LOGIN_DEBUG: Ajax parameter: " . ($_POST['ajax'] ?? 'undefined'));
...
// Traitement inscription coursier
if ($_POST['action'] ?? '' === 'register') {
    try {
        ...
        $type_poste = $_POST['type_poste'] ?? 'coursier_moto';
        
        // Validation
        if (empty($nom) || empty($prenoms) || empty($telephone)) {
            throw new Exception("Nom, prénoms et téléphone sont obligatoires");
        }
        
        // Vérifier si le téléphone existe déjà
        $stmt = $pdo->prepare("SELECT id FROM agents_suzosky WHERE telephone = ?");
        $stmt->execute([$telephone]);
        if ($stmt->fetch()) {
            throw new Exception("Ce numéro de téléphone est déjà enregistré");
        }
        
        // Insertion de la candidature
        $stmt = $pdo->prepare("
            INSERT INTO agents_suzosky 
            (nom, prenoms, date_naissance, lieu_naissance, lieu_residence, telephone, type_poste, statut, created_at) 
            VALUES (?, ?, ?, ?, ?, ?, ?, 'en_attente', NOW())
        ");
        $stmt->execute([
            $nom, $prenoms, $date_naissance, $lieu_naissance, 
            $lieu_residence, $telephone, $type_poste
        ]);
        
        $registerSuccess = "Candidature envoyée avec succès ! Vous recevrez une confirmation par SMS une fois votre dossier validé.";
...
if (($_POST['ajax'] ?? '' === 'true') && !empty($_POST['action'])) {
    ...
    try {
        ...
        switch ($_POST['action']) {
            case 'login':
                // Traitement login AJAX
                $identifier = trim($_POST['identifier'] ?? '');
                $password = trim($_POST['password'] ?? '');
                
                error_log("LOGIN_AJAX_DEBUG: Identifier='$identifier', Password='" . (empty($password) ? 'vide' : $password) . "'");
                ...
                if ($authResult) {
                    ...
                    echo json_encode(['success' => false, 'error' => 'Identifiants incorrects ou compte non activé']);
                }
                exit;
            ...
            case 'get_business_assignments':
                ...
                $stmt = $pdo->prepare("
                    ...
                    WHERE bl.coursier_id = ? 
                    AND DATE(bl.date_livraison_prevue) = ?
                    AND bl.statut IN ('affectee_auto', 'en_cours', 'livree')
                    ORDER BY bl.priorite_geo ASC, bl.date_creation ASC
                ");
                
                $stmt->execute([$coursier_id, $date_livraison]);
            ...
            case 'get_optimized_route':
                // Optimisation de trajet avec Google Maps API
                $date_livraison = $_POST['date'] ?? date('Y-m-d');
                ...
                $stmt = $pdo->prepare("
                    SELECT bl.id, bl.adresse_prise_en_charge, bl.adresse_livraison, 
                           bl.latitude_pickup, bl.longitude_pickup,
                           bl.latitude_delivery, bl.longitude_delivery,
                           bl.nom_client, bl.description_colis
                    FROM business_livraisons bl
                    WHERE bl.coursier_id = ? 
                    AND DATE(bl.date_livraison_prevue) = ?
                    AND bl.statut = 'affectee_auto'
                    ORDER BY bl.priorite_geo ASC
                ");
                ...
                if (!empty($livraisons)) {
                    $optimized_route = optimizeDeliveryRoute($current_lat, $current_lng, $livraisons);
                    echo json_encode($optimized_route);
                } else {
                    echo json_encode(['route' => [], 'total_distance' => 0, 'estimated_time' => 0]);
                }
                exit;
