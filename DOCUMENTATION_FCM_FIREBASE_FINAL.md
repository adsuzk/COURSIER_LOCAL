...
- `Scripts/Scripts cron/fcm_auto_cleanup.php` : signale les tokens très anciens mais ne les désactive plus automatiquement. Utilisez `--apply` pour purger ; sans option, le script reste purement informatif.
...
- **Visibilité immédiate** : le formulaire reste affiché dès qu'au moins un token FCM est actif (`is_active = 1`), même si son `last_ping` remonte à plusieurs heures. Les valeurs `seconds_since_last_active` sont indicatives.
- **Fraîcheur du signal** : `register_device_token_simple.php` met `last_ping = NOW()` à chaque rafraîchissement. Le compte à rebours de verrouillage ne démarre que lorsque `active_count = 0`; `stale_active_count` sert à diagnostiquer les tokens qui devraient être purgés manuellement.
