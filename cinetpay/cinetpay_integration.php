...
class SuzoskyCinetPayIntegration {
    ...
    $this->apiUrl    = $cfg['endpoint']   ?? $this->apiUrl;
    $this->mode      = strtolower($cfg['mode'] ?? $this->mode);
    }
    /**
     * Initier un paiement pour une commande client
     ...
     * @return array
     */
    public function initiateOrderPayment(string $orderNumber, float $amount, string $currency = 'XOF', string $description = '', string $phone = '', string $email = '')
    {
        try {
            $transactionId = 'ORDER_' . $orderNumber . '_' . time() . '_' . rand(1000, 9999);
            $desc = $description ?: 'Paiement commande ' . $orderNumber;
            // Ensure order payments table exists and record pending transaction
            $this->createOrderPaymentTables();
            $this->saveOrderPaymentRecord($transactionId, $orderNumber, $amount, 'pending');
            $paymentData = [
                'apikey' => $this->apiKey,
                'site_id' => $this->siteId,
                ...
                'currency' => $currency,
                'description' => $desc,
                // Ajout d'un paramètre order pour suivre le retour
                'return_url' => $this->getReturnUrl() . '?order=' . urlencode($orderNumber) . '&tid=' . urlencode($transactionId),
                'notify_url' => $this->getNotifyUrl() . '?order=' . urlencode($orderNumber),
                'metadata' => json_encode([
                        'order_number' => $orderNumber,
                        'type' => 'order',
                        'platform' => 'suzosky_coursier_site'
                    ])
            ];
            // Include customer contact if provided
            if ($phone) {
                $paymentData['customer_phone_number'] = $phone;
            }
            if ($email) {
            ...
            throw new Exception('Erreur initiation paiement commande : ' . ($response['message'] ?? 'Inconnue'));
        } catch (Exception $e) {
            error_log('Erreur CINETPAY commande initiation: ' . $e->getMessage());
            return [
                'success' => false,
                'error' => $e->getMessage()
            ];
    ...
    public function checkPaymentStatus($transactionId) {
        try {
            ...
            $response = $this->callCinetPayAPI($checkData, $this->buildCheckUrl());
    ...
    public function handlePaymentNotification($postData) {
        try {
            // Vérification de la signature (fallback sur check API si invalide)
            $signatureOk = $this->verifyNotificationSignature($postData);
            
            $transactionId = $postData['cpm_trans_id'];
            $amount = $postData['cpm_amount'];
            $status = $postData['cpm_result'];
            
            // Si signature invalide, vérifier via API et surcharger status
            if (!$signatureOk) {
            ...
            // Gérer d'abord les paiements de commande (order)
            $orderPayment = $this->getOrderPaymentByTransactionId($transactionId);
            if ($orderPayment) {
                if ($status == '00') {
                    $this->updateOrderPaymentStatus($transactionId, 'completed');
                    // Optionnel: marquer la commande comme payée si colonne existe
                    $this->markOrderAsPaid($orderPayment['order_number'], $amount);
                    return ['status' => 'success', 'message' => 'Paiement commande confirmé'];
                } else {
                    $this->updateOrderPaymentStatus($transactionId, 'failed');
                    return ['status' => 'failed', 'message' => 'Paiement commande échoué'];
                }
            }

            // Sinon, traiter les recharges coursiers historiques
    ...
    logMessage('cinetpay_api.log', 'CINETPAY API Request to ' . $apiUrl . ' Data: ' . json_encode($data));
        ...
        $disableSsl = strtolower((string)(getenv('CINETPAY_DISABLE_SSL_VERIFY') ?: ''));
        $sslBypass = in_array($disableSsl, ['1', 'true', 'yes'], true);
        $shouldVerifySsl = !$sslBypass && $this->mode !== 'stub';
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, $shouldVerifySsl);
        curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, $shouldVerifySsl ? 2 : 0);
        
    $response = curl_exec($ch);
    $httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
    // Log response data
    logMessage('cinetpay_api.log', 'CINETPAY API Response HTTP ' . $httpCode . ' Body: ' . $response);
        ...
        // Accept HTTP 200 and 201 as valid responses
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            throw new Exception('Réponse JSON invalide de CinetPay: ' . $response);
        }
        return $decoded;
    }
    ...
    private function getReturnUrl() {
        // Utiliser le helper centralisé pour gérer local/prod et sous-dossiers
        return appUrl('cinetpay/payment_return.php');
    }
    
    private function getNotifyUrl() {
        return appUrl('cinetpay/payment_notify.php');
    }

    private function buildCheckUrl() {
        $custom = getenv('CINETPAY_CHECK_ENDPOINT');
        if ($custom) {
            return $custom;
        }
    ...
    // ====== Gestion paiements de commandes ======
    private function createOrderPaymentTables() {
        $pdo = getDBConnection();
    ...
    private function saveOrderPaymentRecord($transactionId, $orderNumber, $amount, $status) {
        $pdo = getDBConnection();
        $stmt = $pdo->prepare("INSERT INTO order_payments (transaction_id, order_number, amount, status) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE order_number=VALUES(order_number), amount=VALUES(amount), status=VALUES(status)");
        $stmt->execute([$transactionId, $orderNumber, $amount, $status]);
    }

    private function updateOrderPaymentStatus($transactionId, $status) {
        $pdo = getDBConnection();
        $stmt = $pdo->prepare("UPDATE order_payments SET status = ?, updated_at = NOW() WHERE transaction_id = ?");
        $stmt->execute([$status, $transactionId]);
    }

    private function getOrderPaymentByTransactionId($transactionId) {
        $pdo = getDBConnection();
        $stmt = $pdo->prepare("SELECT * FROM order_payments WHERE transaction_id = ?");
        $stmt->execute([$transactionId]);
        return $stmt->fetch();
    }

    private function markOrderAsPaid($orderNumber, $amount) {
        try {
            $pdo = getDBConnection();
            // Détecter le nom de colonne du numéro de commande
            ...
            // Marquer comme payé selon les colonnes disponibles
            if (in_array('paiement_confirme', $cols)) {
                $stmt = $pdo->prepare("UPDATE commandes SET paiement_confirme = 1 WHERE {$orderNumCol} = ?");
                $stmt->execute([$orderNumber]);
            } elseif (in_array('statut_paiement', $cols)) {
                $stmt = $pdo->prepare("UPDATE commandes SET statut_paiement = 'paye' WHERE {$orderNumCol} = ?");
                $stmt->execute([$orderNumber]);
            }
        } catch (Exception $e) {
            // Journaliser mais ne pas bloquer le flux
            error_log('markOrderAsPaid error: ' . $e->getMessage());
        }
    }
    ...
    private function getCourrierName($coursierId) {
        ...
        $stmt = $pdo->prepare("SELECT nom FROM coursiers WHERE id = ?");
    ...
    private function getCourrierPhone($coursierId) {
        ...
        $stmt = $pdo->prepare("SELECT telephone FROM coursiers WHERE id = ?");
        $stmt->execute([$coursierId]);
